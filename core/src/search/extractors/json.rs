...
pub fn json_single_extract(path: &str) -> Result<JsonPathSingleExtract, String> {
    Ok(JsonPathSingleExtract {
        path: JsonPathFinder::from_str(r"{}", path)?,
    })
}

pub fn json_multi_extract(path: &str) -> Result<JsonPathMultiExtract, String> {
    Ok(JsonPathMultiExtract {
        path: JsonPathFinder::from_str(r"{}", path)?,
    })
}
        ...
        match msg.payload_view::<str>() {
            ...
            Some(Ok(json)) => {
                if let Ok(value) = serde_json::from_str::<Value>(json) {
                    ...
                    let matched = found.as_array().and_then(|values| values.first().cloned());
                    Ok(matched)
                } else {
                    Ok(None)
                }
...
mod tests {
    ...
    use crate::tests::{
        clean, collect_search_notifications, prepare, produce_json_records, test_cluster_config,
        NestedTestRecord, TestRecord,
    };
    use crate::utils::all_matches;
    use crate::{search_topic, ChronoDuration};
    ...
    fn test_single_extract() {
        ...
        let json_payload = Some(
            serde_json::to_string(&payload)
                .expect("Could not serialize test record")
                .into_bytes(),
        );
        let record = OwnedMessage::new(
            ...
            "topic".to_string(),
            Timestamp::CreateTime(Utc::now().timestamp_millis()),
            0,
            0,
            None,
        );
    ...
    fn test_multi_extract() {
        let mut extractor =
            json_single_extract("$.nested.ints").expect("Could not create JSON path");
        let key = "some";
        let i = 4;
        let payload = TestRecord {
            key: key.to_string(),
        ...
        assert_eq!(
            ...
            Ok(Some(serde_json::json!(vec![i, i + 1])))
        );
    }
    ...
    async fn test_single_match_from_kafka() {
        ...
        let all_notifs: Vec<SearchNotification> = futures::future::join_all(tasks)
            .await
            .into_iter()
        ...
        assert!(
            matches!(first, SearchNotification::Prepare(_)),
            "The first notification received must be a Prepare notification"
        );

        let finished = all_notifs
            .last()
            .expect("Could access the last received notification");
        assert!(
            matches!(finished, SearchNotification::Finish(_)),
            "The last notification received must be the Finished notification"
        );
        ...
        assert!(
            progresses_received.len() > 1,
            "Multiple progress notifications must have been received"
        );
        ...
        for progress in progresses_received {
            ...
            assert!(
                progress.overall_progress.done >= last_nb_read,
                "Progress notifications must have been received in order"
            );
            last_nb_read = progress.overall_progress.done;
            assert!(
                progress.matches >= last_nb_matches,
                "Progress notifications must have been received in order"
            );
        ...
        let matched_payload = matches[0]
            .payload_view::<str>()
            .expect("Could not read payload from matched msg")
            .expect("Could not read payload from matched msg");
        let matched: TestRecord =
            serde_json::from_str(matched_payload).expect("Could not parse JSON matched");
